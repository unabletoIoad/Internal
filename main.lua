...
-- Always force background transparent for text labels
...
LUAX["6"].TextSize = 14
-- Default red indicates OFF
LUAX["6"].BackgroundColor3 = Color3.fromRGB(255, 0, 0)
-- For toggle buttons, force background visible.
...
-- The icon's background remains forced transparent
...
-- Set initial image transparency for fading
...
-- Fade in the entire ScreenGui smoothly when executed
...
-- Make the UI draggable via the TopBar with a smooth delayed glide
...
-- Hide/Show UI with Right Ctrl key using a fade in/out effect.
-- TextLabels always keep their BackgroundTransparency = 1,
-- Toggle buttons ("Enable") always keep their BackgroundTransparency = 0.
...
local function setupToggle(button, name)
    local toggled = false -- false = off; true = on
    button.MouseButton1Click:Connect(function()
        toggled = not toggled
        if toggled then
            button.BackgroundColor3 = Color3.new(0, 1, 0) -- green for on
            print(name .. " toggled ON")
        else
            button.BackgroundColor3 = Color3.new(1, 0, 0) -- red for off
...
-- Change load from require to loadstring (or another method as needed)
